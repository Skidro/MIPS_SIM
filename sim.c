...
uint8_t instruction_decode(void)
{
    ...
    SIM_DEBUG(printf("opcode : 0x%.2x (%.2d) | rs : 0x%.2x (%.2d) | rt : 0x%.2x (%.2d) | rd : 0x%.2x (%.2d) | shamt : 0x%.2x (%.2d) | funct : 0x%.2x (%.2d) | immediate : 0x%.4x (%.5d)\n", opcode, opcode, rs, rs, rt, rt, rd, rd, shamt, shamt, funct, funct, immediate, immediate));
