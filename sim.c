 ...
 * MIPS Simulator in C
 * 
 * This is a simple simulator which models different execution stages
 * of a MIPS ISA Processor
 *
...
uint8_t instruction_decode(void)
{
    ...
    SIM_DEBUG(printf("opcode : 0x%x | rs : 0x%x | rt : 0x%x | immediate : 0x%x\n", opcode, rs, rt, immediate));
    ...
    if (status == SUCCESS) {
        ...
        jump_shift.input_1              = jump;
        jump_shift.input_2              = (int32_t)2;
    ...
    if (status == SUCCESS) {
        ...
        immd_shift.input_1              = w_signext_immd;
        immd_shift.input_2              = (int32_t)2;
    ...
    if (status == SUCCESS) {
        ...
        /* Write the required data to the input of alu_in mux */
        mux_alu_in.input_0              = w_reg_file_data_2;
        mux_alu_in.input_1              = w_signext_immd;
        mux_alu_in.control_signal       = w_ctl_alu_src;

        /* Execute the multiplexor process for alu_in mux */
        status = multiplexor_process(&mux_alu_in, &w_mux_alu_in);

        //printf("mux->ctl        : 0x%x\n", mux_alu_in.control_signal);
        //printf("w_mux_alu_in    : 0x%x\n", w_mux_alu_in);
    }

    if (status == SUCCESS) {
        /* Write input data for ALU */
        alu.input_1                     = w_reg_file_data_1;
        alu.input_2                     = w_mux_alu_in;
        alu.alu_ctl                     = w_alu_ctl_in;                 

        SIM_DEBUG(printf("\nalu->input1 : 0x%x\n", alu.input_1));
        SIM_DEBUG(printf("alu->input2 : 0x%x\n", alu.input_2));
     }
...
uint8_t instruction_execute(void)
{
    ...
    SIM_DEBUG(printf("\nalu->output  : 0x%x\n", w_alu_out_1));
    SIM_DEBUG(printf("alu->zero    : 0x%x\n", w_zero));
...
uint8_t generate_control_signals(void)
{
    ...
    if (status == SUCCESS)
    {
        ...
        w_ctl_alu_src       = ctl_unit.alu_src;
 ...
 * NOTE: The register file reads on clock-hi and writes on clock-low
 */
