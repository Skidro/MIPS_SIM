 ...
 * Description  :       This file contains the macro definitions used by the 
 *                      hardware code for pirnting the execution data
...
#define rdump_map(fd, core_dump) 											\
	do {														\
		...
		for(i = 0; i < MIPS_ARCH_REGS/4; i++) {									\
			for (j = 0; j < 4; j++) {									\
                		fprintf(fd, "R%02d: 0x%08x (%10d)", 							\
					((i*4) + j), core_dump[((i*4)+j)], core_dump[((i*4)+j)]);			\
        		}												\
        		fprintf(fd, "\n");										\
    		}													\
															\
